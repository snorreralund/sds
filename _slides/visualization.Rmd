...
```{r}
library("readr")
df = read_csv("https://raw.githubusercontent.com/sebastianbarfort/sds_summer/gh-pages/data/polls_tidy.csv")
names(df)
```
...
p = ggplot(df, aes(x = predicted, y = density, 
                   group = group, color = Parti))
p = p + geom_line() + 
  facet_grid(party.ord ~ ., scales = "free_y", switch = "y") 
```
...
```{r, echo = FALSE}
ggsave(plot = p, file = "figures/polls.pdf",
       width = 8, height = 6)
```
