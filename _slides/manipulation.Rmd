...
\centering
We will do Exercise 4.5.1 (*Prediction Based on Betting Markets*) in Imai (2016) using `for` loops. 
...
### Number of misclassified states in 2008
...
## Imai Exercise: 4.5.1.2

```{r}
df.08.1 = df.08 %>% 
  filter(days.to.election > 0 & 
           days.to.election <= 90) %>%
  filter(obama.intmarg > 0) %>%
  group_by(days.to.election) %>% 
  summarise(
...
## Imai Exercise: 4.5.1.3

```{r}
library("RcppRoll") # for 'roll_mean' function
df.08.2 = df.08 %>% 
  filter(days.to.election > 0 & 
         days.to.election <= 90) %>%
  arrange(state, days.to.election) %>% 
  group_by(state) %>% 
  mutate(
    roll.mean = roll_mean(x = obama.intmarg, 7, 
                              align = "right", 
                              fill = NA)) %>% 
  ungroup() %>%
  filter(roll.mean > 0) %>%
...
```{r, echo = FALSE}
p = ggplot(df.08.2, aes(x = days.to.election, 
                        y = EV))
p = p + 
  geom_line() + 
  geom_hline(yintercept = 370, 
